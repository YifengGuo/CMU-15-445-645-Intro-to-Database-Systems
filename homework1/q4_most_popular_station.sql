...
-- As above, either starting a trip or finishing a trip at a station, 
-- the trip is counted as one "visit" to that station. The trip is only counted once if the start station and the end station are the same.
-- Details: For each station, print city name, most popular station name and its visit count. Sort by city name, ascending.


-- count of start station's visits with different end and start station
WITH start_count AS (
